...
const Components = {
  ...
  // =========================================================================
    ...
    // --- VISTA PARA PADRES DE FAMILIA Y ESTUDIANTES: BOLETA OFICIAL DE CALIFICACIONES ---
    if (role === 'estudiante' || role === 'padre') {
      const currentUser = state.currentUser || {};
      let studentName = currentUser.studentName || currentUser.name || "FELICIANO CABALLERO, THIAGO JESUS";
      let studentGrade = currentUser.gradeLevel || currentUser.grade || currentUser.detail || "2° de Secundaria";
      let studentCode = currentUser.studentCode || currentUser.code || currentUser.id || "EST-2026-770";
      let parentName = currentUser.name || "Sr. Feliciano";

      if (currentUser.children && currentUser.children.length > 0) {
        const selectedId = currentUser.selectedChildId || currentUser.children[0].id;
        const child = currentUser.children.find(c => c.id === selectedId) || currentUser.children[0];
        studentName = child.name || studentName;
        studentGrade = child.grade || studentGrade;
        studentCode = child.id || studentCode;
      }

      // Cuadros de calificaciones por competencia
      const gradesTable = [
        { area: 'Matemática (Álgebra, Geometría y Raz. Matemático)', c1: 18, c2: 19, c3: 18, b1: 18, b2: 18, b3: 19, b4: '-' },
        { area: 'Comunicación y Literatura', c1: 17, c2: 18, c3: 19, b1: 17, b2: 18, b3: 18, b4: '-' },
        { area: 'Ciencia y Tecnología (Física, Química y Robótica)', c1: 19, c2: 20, c3: 19, b1: 19, b2: 19, b3: 20, b4: '-' },
        { area: 'Ciencias Sociales e Historia del Perú', c1: 17, c2: 17, c3: 18, b1: 16, b2: 17, b3: 18, b4: '-' },
        { area: 'Inglés Técnico Avanzado', c1: 19, c2: 18, c3: 19, b1: 18, b2: 19, b3: 19, b4: '-' },
        { area: 'Educación para el Trabajo / Computación', c1: 20, c2: 20, c3: 20, b1: 20, b2: 20, b3: 20, b4: '-' },
        { area: 'Educación Física y Deportes', c1: 18, c2: 19, c3: 19, b1: 18, b2: 18, b3: 19, b4: '-' },
        { area: 'Arte y Cultura', c1: 18, c2: 18, c3: 18, b1: 17, b2: 18, b3: 18, b4: '-' },
        { area: 'Educación Religiosa y Valores', c1: 19, c2: 19, c3: 19, b1: 19, b2: 19, b3: 19, b4: '-' },
        { area: 'Tutoría y Orientación Educativa (Conducta)', c1: 'AD', c2: 'AD', c3: 'AD', b1: 'AD', b2: 'AD', b3: 'AD', b4: '-' }
      ];

      return `
        <div class="fade-in">
          <!-- Encabezado de la Boleta Oficial -->
          <div class="card" style="margin-bottom: var(--space-4); background: linear-gradient(135deg, #1e3a8a, #0b132b); color: white; border: none; padding: 20px 24px;">
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
              <div>
                <div style="font-size: 11px; color: var(--color-yellow-300); font-weight: 800; text-transform: uppercase;">
                  MINEDU • SIAGIE 2026 • UGEL 05 • I.E.P. "EL EDUCADOR"
                </div>
                <h2 style="font-size: 20px; font-weight: 900; margin: 4px 0 0; color: white;">
                  📊 Boleta Oficial de Información del Estudiante
                </h2>
                <p style="font-size: 12px; color: #cbd5e1; margin: 4px 0 0;">
                  Calificaciones bimestrales oficiales y progreso por competencias del año lectivo 2026.
                </p>
              </div>
              <button class="btn btn-gold" onclick="window.print()" style="font-weight: 900; padding: 10px 22px; cursor: pointer; display: flex; align-items: center; gap: 6px; box-shadow: 0 4px 12px rgba(245,158,11,0.4);">
          ...
          <div class="card" style="margin-bottom: var(--space-4); padding: 16px 20px;">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 12px; font-size: 13px;">
              <div><strong style="color: #64748b;">Estudiante:</strong> <span style="color: #1e3a8a; font-weight: 900;">${studentName}</span></div>
              <div><strong style="color: #64748b;">Grado y Sección:</strong> <span style="font-weight: 800; color: #0f172a;">${studentGrade} "A"</span></div>
              <div><strong style="color: #64748b;">Código de Matrícula:</strong> <span style="font-weight: 800; color: #0f172a;">${studentCode}</span></div>
              <div><strong style="color: #64748b;">Bimestre Actual:</strong> <span class="status-badge" style="background: #ecfdf5; color: #047857; font-weight: 800;">III Bimestre 2026</span></div>
            </div>
          </div>

          <!-- Tabla de Calificaciones Oficiales -->
          <div class="card" style="padding: 0; overflow-x: auto; margin-bottom: var(--space-4);">
            <table style="width: 100%; border-collapse: collapse; font-size: 12.5px;">
              <thead>
                <tr style="background: #1e3a8a; color: white;">
                  <th style="padding: 10px 14px; text-align: left; border: 1px solid #cbd5e1;">Área Curricular / Competencias</th>
                  <th style="padding: 10px; width: 65px; text-align: center; border: 1px solid #cbd5e1;">I Bim</th>
                  <th style="padding: 10px; width: 65px; text-align: center; border: 1px solid #cbd5e1;">II Bim</th>
                  <th style="padding: 10px; width: 65px; text-align: center; border: 1px solid #cbd5e1; background: #f59e0b; color: #0b132b; font-weight: 900;">III Bim</th>
                  <th style="padding: 10px; width: 65px; text-align: center; border: 1px solid #cbd5e1;">IV Bim</th>
                  <th style="padding: 10px; width: 75px; text-align: center; border: 1px solid #cbd5e1; background: #0b132b;">Promedio</th>
                </tr>
              </thead>
              <tbody>
                ${gradesTable.map((row, idx) => {
                  const prom = typeof row.b3 === 'number' ? Math.round((row.b1 + row.b2 + row.b3) / 3) : row.b3;
                  return `
                    <tr style="background: ${idx % 2 === 0 ? '#ffffff' : '#f8fafc'};">
                      <td style="padding: 9px 14px; border: 1px solid #cbd5e1; font-weight: 700; color: #1e293b;">
                        ${row.area}
                      </td>
                      <td style="padding: 8px; text-align: center; border: 1px solid #cbd5e1; font-weight: 800; color: #1e40af;">${row.b1}</td>
                      <td style="padding: 8px; text-align: center; border: 1px solid #cbd5e1; font-weight: 800; color: #1e40af;">${row.b2}</td>
                      <td style="padding: 8px; text-align: center; border: 1px solid #cbd5e1; font-weight: 900; color: #047857; background: #ecfdf5;">${row.b3}</td>
                      <td style="padding: 8px; text-align: center; border: 1px solid #cbd5e1; color: #94a3b8;">${row.b4}</td>
                      <td style="padding: 8px; text-align: center; border: 1px solid #cbd5e1; font-weight: 900; color: #15803d; background: #f0fdf4;">${prom}</td>
                    </tr>
                  `;
                }).join('')}
              </tbody>
          ...
          <!-- Conclusión Descriptiva y Sello Digital -->
          <div class="card" style="padding: 18px 20px;">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 16px; align-items: center; flex-wrap: wrap;">
              <div>
                <h4 style="font-size: 13px; font-weight: 900; color: #1e3a8a; margin: 0 0 6px;">
                  📝 Conclusión Descriptiva del Tutor(a):
                </h4>
                <p style="font-size: 12.5px; color: #334155; line-height: 1.6; margin: 0; font-style: italic;">
              ...
              <div style="text-align: center; border: 1px dashed #cbd5e1; border-radius: 8px; padding: 10px; display: flex; flex-direction: column; align-items: center;">
                <div style="font-size: 10px; color: #64748b; font-weight: 800; margin-bottom: 4px;">SELLO DIGITAL IEP EL EDUCADOR</div>
                <div style="width: 70px; height: 70px;">${this.generateQRSVG(studentCode, 70)}</div>
                <span style="font-size: 9.5px; color: #15803d; font-weight: 800; margin-top: 2px;">✓ Verificado SIAGIE</span>
              </div>
            </div>
          </div>
        </div>
    ...
    if (role === 'auxiliar') {
      return `
        <div class="fade-in card" style="padding: 50px 20px; text-align: center; max-width: 600px; margin: 40px auto;">
          <div style="font-size: 48px; margin-bottom: 12px;">🔒</div>
          <h2 style="font-size: 18px; font-weight: 900; color: var(--color-navy-900);">Módulo Exclusivo de Docentes</h2>
          <p style="font-size: 13px; color: var(--text-muted);">El registro de calificaciones es de uso exclusivo del personal docente.</p>
        </div>
      `;
    }
